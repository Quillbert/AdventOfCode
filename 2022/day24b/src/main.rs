...
fn solve(state: State, start: (usize, usize), end: (usize, usize)) -> (usize, State) {
	...
	while !queue.is_empty() {
		...
		if place.0 > 1 && place.1 < states[0].space.len()-1 {
...
fn main() {
	...
	let end = (state.space[0].len()-2, state.space.len()-1);
    let (first, state) = solve(state, (1,0), end);
	let (second, state) = solve(state, end, (1, 0));
	let (third, state) = solve(state, (1,0), end);
